...
# and download 'trecis2018-A', 'trecis2018-B', 'trecis2019-A' and 'trecis2019-B' sets'
	...
	if not files.startswith('.'):
		with open(PATH_tweets+files, "r",encoding='UTF-8') as file:
			for line in file:
				...
				if str(tweet["id"]) in tweet_id_map:
					tweet_id_map[str(tweet["id"])]["text"] = tweet["text"]
					if "created_at" in tweet:
						if "+0000" in tweet["created_at"]:
							tweet_id_map[str(tweet["id"])]["timestamp"] = time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(tweet["created_at"],'%a %b %d %H:%M:%S +0000 %Y'))
						else:
							tweet_id_map[str(tweet["id"])]["timestamp"] = time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(tweet["created_at"],'%b %d, %Y %H:%M:%S %p'))
					else:
						if "+0000" in tweet["createdAt"]:
							tweet_id_map[str(tweet["id"])]["timestamp"] = time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(tweet["createdAt"],'%a %b %d %H:%M:%S +0000 %Y'))
						else:
							tweet_id_map[str(tweet["id"])]["timestamp"] = time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(tweet["createdAt"],'%b %d, %Y %H:%M:%S %p'))
					if "user.id_str" in tweet:
						tweet_id_map[str(tweet["id"])]["user_id_str"] = str(tweet["user"]["id"])
					else:
						tweet_id_map[str(tweet["id"])]["user_id_str"] = str(tweet["user"]["id"])
...
for i in df["content.event"].unique():
	...
	for enum,j in enumerate(df[df["content.event"]==i]["content.text"]):
		with open('./Sets/All tweets/Raw/'+i+'.txt','a',encoding='UTF-8') as file:
			file.write(with_surrogates(cleanRaw(str(j)))+'\n')
...
for i in df["content.event"].unique():
	...
	for j in df[df["content.event"]==i]["content.text"]:
		with open('./Sets/All tweets/PreProcessed/'+i+'.txt','a',encoding='UTF-8') as file:
			file.write(with_surrogates(cleanPreProcessed(str(j)))+'\n')
...
for i in df_summary["content.event"].unique():
	...
	for j in df_pku[df_pku["content.event"]==i]["content.text"]:
		...
		with open('./Sets/PKUSet/'+i+'/'+str(cpt)+'.txt','a',encoding='UTF-8') as file:
			file.write(with_surrogates(cleanPreProcessed(str(j)))+'<END_OF_TWEET>')
		cpt+=1
...
for i in df_pku["content.event"].unique():
	...
	for j in df_pku[df_pku["content.event"]==i]["content.text"]:
		with open('./Sets/Tweets with ids/'+i+'.txt','a',encoding='UTF-8') as file:
			file.write(str(l_timestamp[cpt])+'\t'+str(l_ids[cpt])+'\t'+str(l_uids[cpt])+'\t'+with_surrogates(cleanPreProcessed(str(j)))+'\n')
		cpt+=1
    ...
    for k in df_randoms["content.event"].unique():
        ...
        for l in df_randoms[df_randoms["content.event"]==k]["content.text"]:
            with open('./Reproduce results/Models Summarization/'+str(i)+'/'+k+'.txt','a',encoding='utf-8') as file:
                file.write(with_surrogates(cleanPreProcessed(str(l)))+'\n')
            with open('./Sets/Random Summaries Raw/'+str(i)+'/'+k+'.txt','a',encoding='utf-8') as file:
                file.write(with_surrogates(cleanRaw(str(l)))+'\n')
